...
import { PaperAirplaneIcon, LinkIcon } from './Icons';
            ...
            <div className="flex-grow p-4 overflow-y-auto space-y-4">
                ...
                {messages.map(msg => (
                    <div key={msg.id} className={`flex ${msg.sender === 'user' ? 'justify-end' : 'justify-start'}`}>
                        <div className={`max-w-xs md:max-w-sm p-3 rounded-2xl ${msg.sender === 'user' ? 'bg-blue-600 text-white rounded-br-lg' : 'bg-gray-100 text-gray-800 rounded-bl-lg'}`}>
            ...
            <div className="p-4 border-t bg-white">
                <div className="flex items-center space-x-2">
                    <input
                        ...
                        className="flex-grow border-gray-300 rounded-full shadow-sm focus:ring-blue-500 focus:border-blue-500 py-2 px-4"
                        disabled={isLoading}
                    />
                    <button onClick={handleSend} disabled={isLoading || !input.trim()} className="bg-blue-600 text-white rounded-full p-2 disabled:bg-gray-400">
                        <PaperAirplaneIcon />
                    </button>
                </div>
            </div>
